...
windower.register_event('addon command', function(...)
	...
	if commands[1] == 'help' then
		print(' geo indi [spell]       geo indi precision"  ')
		print(' geo indi off')
		print(' geo geo [spell]        e.g //geo geo precision  ')
		print(' geo geo off')
		print(' geo entrust player [spell] e.g geo entrust kupipi refresh')
		print(' geo entrust off')
		print(' geo refresh kupipi [on/off]   -- /ma refresh kupipi ')
		print(' geo haste tenzen [on/off]     -- /ma haste tenzen')
		print(' geo recast indi [min] [max]   -- Begin recasting indi between [min] and [max] seconds before they wear off.(randomized)')
		print(' geo recast buff [min] [max]   -- Same as the above for haste and refresh.')
		print(' geo aug lifestream 20         -- set indi effect duration augment on lifestream cape')
		print(' geo active                    -- display active settings in text box')
		print(' geo save                      -- save settings on per character basis')
		print(' geo [on/off]                  -- turn actions on/off')
	end
